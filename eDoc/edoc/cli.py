...
async def _cmd_sign(args: argparse.Namespace, config: Config) -> int:
    ...
    if failures and not args.dry_run:
        console.print(
            f"[yellow]Leaving {args.file} in place because {len(failures)} document(s) failed.[/yellow]"
        )
        logging.warning("Not archiving queue because %d document(s) failed", len(failures))
